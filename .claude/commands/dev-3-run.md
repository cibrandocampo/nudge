...
argument-hint: <task-id, e.g.: T001, T005_celery>
---
...
2. **Before writing**: read the affected files. Understand existing patterns. Follow conventions from the `backend-patterns` and `frontend-patterns` skills.
...
**All commands via Docker** — use `docker compose -f dev/docker-compose.yml exec`.
...
|------|---------|-------------|
| Backend tests | `docker compose -f dev/docker-compose.yml exec backend python manage.py test 2>&1` | "OK", 0 failures |
| Frontend tests | `docker compose -f dev/docker-compose.yml exec frontend npx vitest run 2>&1` | All pass |
| Backend lint | `docker compose -f dev/docker-compose.yml exec backend ruff check . 2>&1` | No errors |
| Frontend lint | `docker compose -f dev/docker-compose.yml exec frontend npx eslint src/ 2>&1` | No errors |
| Frontend build | `docker compose -f dev/docker-compose.yml exec frontend npm run build 2>&1` | Exit 0 |
